...
class HomeViewComponents {
    ...
    renderFeaturedArtists(){
        const artistsContainer = Html().create("div").addClass("artist-container");
        const artistsHeader = Html().create("h2").addClass("artist_container--header").text("Featured Artists")
        ...
        Api().getRequest(`http://localhost:8080/api/artists`, (responseObject) => {
            responseObject.forEach((artist) =>{
                const artistBox = Html().create("div").addClass("artist-box");
                const artistFigure = Html().create("figure").addClass("artist-cover")
                const artistImg = Html().create("img").addClass("avatar-figure__img-artist").addAttribute("src", artist.imgUrl)
                const artistName = Html().create("h3").text(artist.name);
    ...
    renderFeaturedAlbums(){
        const albumsContainer = Html().create("div").addClass("album-container");
        const albumsHeader = Html().create("h2").addClass("album_container--header").text("Featured Albums")
        ...
        Api().getRequest(`http://localhost:8080/api/albums`, (responseObject) => {
            responseObject.forEach((album) =>{
                const albumBox = Html().create("div").addClass("album-box");
                const albumFigure = Html().create("figure").addClass("album-cover")
                const albumImg = Html().create("img").addClass("avatar-figure__img-artist").addAttribute("src", album.imgUrl)
                const albumTitle = Html().create("h3").text(album.title);
    ...
    renderFeaturedSongs(){
        const songsContainer = Html().create("div").addClass("song-container")
        const songsHeader = Html().create("h2").addClass("song_container--header").text("Featured Songs")
